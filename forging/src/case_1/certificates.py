...
def gen_certificate(username: str, website_name: str, expiration_date: str):
    cert = {"name": username, "website": website_name, "expdate": expiration_date}
    return json.dumps(cert)


def sign_certificate_v1(signature_scheme: DSA, certificate: str, nonce):
...
while GCD(nonce, sig_scheme.q) != 1:
    nonce = random.randint(2, sig_scheme.q - 1)

print("Nonce used for signing:", nonce)
